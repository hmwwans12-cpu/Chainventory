-- ============================================================================
-- Chainventory - 0028: RPC inventory hardening (audit v0.1.1)
-- ============================================================================
-- C-01 : reject quantity null/negatif/nol di level RPC (bypass API).
-- H-01 : produk ARCHIVED tidak menerima movement baru.
-- H-04 : reversal target WAJIB stock_in/stock_out committed -
--        reversal of reversal ditolak (INVALID_REVERSAL).
...
begin
  ...
  -- Warehouse suspended menolak SEMUA mutation (PRD Ôö¼┬║12 gap closure).
  ...
  -- Balance row lock (SELECT ... FOR UPDATE) ├ö├ç├Â hanya untuk tipe yang langsung
  ...
  -- Notifikasi: adjustment butuh approval ├ö├Ñ├å OWNER + MANAGER.
  ...
  -- Adjustment ├ö├Ñ├å proof dibuat saat approve (movement belum committed).
