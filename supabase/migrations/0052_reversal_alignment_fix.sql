-- ============================================================================
-- Chainventory — 0052: reversal warehouse alignment fix (audit Bagian A A3)
-- ============================================================================
-- Bug 0050: rewrite penuh apply_stock_movement merusak kontrak kanonis
-- 0038/0040 — signature p_expected_balance_version integer (semula bigint),
-- RETURNS stock_movements (semula TABLE(...)), INSERT ke kolom non-eksisten
-- payload/payload_hash, hilangkan ON CONFLICT scope, fingerprint check,
-- proof pipeline, audit. Postgres membuat overload kedua → resolusi RPC
-- ambigu, semua Stock In/Out pecah bila termigrasi.
--
-- Fix: DROP overload 0050, kembalikan definisi kanonis 0040 UTUH, lalu
-- terapkan M-11 sebagai patch kecil: reversal lookup wajib
-- `AND warehouse_id = p_warehouse_id` (defense-in-depth, UUID global unik
-- sehingga collision mustahil dalam praktik).
-- ============================================================================

-- 1. Hapus overload rusak 0050 (integer version, returns stock_movements) ----
...
-- 2. Kembalikan kontrak kanonis 0040 + patch M-11 -----------------------------
...
BEGIN
  ...
  -- M-11 (0052): lookup wajib mencakup warehouse_id (defense-in-depth).
  ...
  IF p_movement_type IN ('stock_in', 'stock_out', 'reversal') THEN
    ...
    ELSIF p_movement_type = 'reversal' THEN
      -- M-11: scope warehouse agar konsisten dengan lookup di atas.
      SELECT movement_type INTO v_original_type
      FROM public.stock_movements
      WHERE id = p_reversal_of AND warehouse_id = p_warehouse_id;
...
comment on function public.apply_stock_movement(
  ...
  'Applies a stock movement atomically. Canonical 0038/0040 + M-11 (0052): reversal lookup includes warehouse_id.';
