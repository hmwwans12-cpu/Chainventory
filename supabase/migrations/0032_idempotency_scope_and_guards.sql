-- ============================================================================
-- Chainventory — 0032: idempotency scope + wallet verified + intent guard
-- ============================================================================

-- 1. apply_stock_movement: idempotency lookup di-scope ke warehouse_id
--    (audit P1-01: sebelumnya global, key sama di warehouse lain = false IDEMPOTENT)
...
BEGIN
  ...
  -- C-01: quantity wajib > 0 (RPC dapat dipanggil langsung oleh user authenticated)
  ...
  -- Warehouse suspended menolak SEMUA mutation.
  ...
  -- Idempotency: scoped ke warehouse_id agar key sama di warehouse lain tidak bentrok
  ...
  -- Product milik warehouse + AKTIF (H-01: archived tidak menerima movement baru)
  ...
  -- Reversal: target committed, product sama, tipe in/out, cumulative limit
  IF p_movement_type = 'reversal' THEN
    ...
    -- P0-01: lock original movement row untuk mencegah concurrent reversal race
