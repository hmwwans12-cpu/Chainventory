...
5. **Wallet migration/ownership transfer on-chain** untuk Owner — ✅ alur end-to-end dibangun 2026-09-13: dialog pilih member → preview wallet primary verified → sign `transferOwnership` via Privy → verifikasi tx on-chain di BFF → RPC `confirm_ownership_transfer` sinkron DB atomik (migrasi 0060, ter-apply live). Kontrak `Warehouse.transferOwnership()` + callback factory sudah ada; yang dibangun: signing, verifikasi, sinkron, i18n `ownership.*`.
...
7. **Redeploy Factory solc 0.8.36** — ✅ v2 terdeploy (`0x3811...8Bf48`, tx `0xdc249aca…`) & tercatat di registry. Switch dieksekusi 2026-09-13: runbook di WORKFLOW §5.1 (verifikasi on-chain proofRecorder + broadcast receipt + kronologi source), `.env.local` lokal sudah menunjuk v2. TERSISA manual: alihkan `WAREHOUSE_FACTORY_ADDRESS` di Vercel Production + redeploy, lalu smoke test warehouse baru. v1 tetap melayani warehouse eksisting (tanpa migrasi data — kontrak immutable).
...
9. ~~**SUPABASE_MANAGEMENT_TOKEN** kedaluwarsa (401)~~ ✅ (2026-09-13) — klaim kedaluwarsa ternyata basi: token di `.env.local` masih valid (Management API 201). Runbook generate/rotasi kini ada di WORKFLOW.md §4.2.
...
18. **Audit 0.1.6 → rilis 0.1.7 (inventory atomicity + fingerprint hardening, 2026-08-24)**:
    ...
    - ~~P1-05/P2 reversal test source-inspection~~ ✅ live integration test: dua request paralel nyata, pemenang commit + yang kalah `INVALID_REVERSAL`, saldo akhir terverifikasi.
    - ~~P2 bulk deterministic idempotency~~ ✅ key `bulk:<wh>:<productId>:initial-stock` (retry/refresh aman).
