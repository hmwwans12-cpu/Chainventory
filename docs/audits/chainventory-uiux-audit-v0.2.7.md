...
`components/ui/card.tsx:15` — `rounded-xl` (16px) on every card. DESIGN §9 says: _"use `rounded-lg` for BOTH controls and cards/panels app-wide — including marketing — so radius stays consistent. Avoid mixing rounded-2xl / rounded-xl / arbitrary rounded-[..] across surfaces."_
