...
```tsx
<Badge className="size-6 text-[10px] tabular-nums ... ...">
...
```tsx
<p className="text-muted-foreground truncate text-xs">{cat.description}</p>
```
...
```tsx
<p className="text-muted-foreground truncate text-sm">{cat.description}</p>
```
